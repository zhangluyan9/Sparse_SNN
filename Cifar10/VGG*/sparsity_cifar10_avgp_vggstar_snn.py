...
load_name = 'merge_ours_baseline.pt'
...
def create_spike_input_cuda(input,T):
    ...
    #1-0.00001
    out = catCuda.getSpikes(out, 1-0.00001)
    return out
    ...
    def _make_layers(self, cfg):
        ...
        for x in cfg:
            if x == 'M':
                ...
                layers += [nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=padding, bias=self.bias),Clamp_q_(),nn.Dropout2d(0.1)]
                in_channels = out_channels

        layers += [nn.AvgPool2d(kernel_size=1, stride=1)]
        return nn.Sequential(*layers) 

class CatVGG_o_(nn.Module):
    def __init__(self, vgg_name, T, is_noise=False, bias=True):
        super(CatVGG_o_, self).__init__()
        ...
        self.classifier2 =self.snn.dense((1, 1, 512), 10,bias = True)
...
def main():
    ...
    model_snn = CatVGG_o('o', T= q_level,bias =True).to(device)
